...
page_fault (struct intr_frame *f) 
{
  ...
  if (!not_present)
    exits(-1, NULL);
  struct vm_entry* vme = find_vme(fault_addr); // check validation of fault_addr
  ...
  bool load_succ = handle_mm_fault(vme); // define in process.c
  //printf("load succ\n");
  if (!load_succ)
  {
    exits(-1, NULL);
...
verify_stack (int32_t addr, int32_t esp)
{
  ...
  bool stack = 0xC0000000 - addr <= 8 * 1024 * 1024; // maximum size is 8MB
